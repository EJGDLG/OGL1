...
class Camera(object):
  ...
  def GetViewMatrix(self):
    if not self.usingLookAt:
      ...
      pitchMat = glm.rotate(identity, radians(self.rotation.x), glm.vec3(1, 0, 0))
      yawMat = glm.rotate(identity, radians(self.rotation.y), glm.vec3(0, 1, 0))
      rollMat = glm.rotate(identity, radians(self.rotation.z), glm.vec3(0, 0, 1))
  ...
  def CreateProjectionMatrix(self, fov, nearPlane, farPlane):
    self.projectionMatrix = glm.perspective(radians(fov), self.screenWidth / self.screenHeight, nearPlane, farPlane)
